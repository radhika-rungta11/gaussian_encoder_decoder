...
def _build_text_report(input_path, output_path, report, psnr, encode_seconds, decode_seconds):
    lines = []
    lines.append("=" * 78)
...
def _build_markdown_report(input_path, output_path, report, psnr, encode_seconds, decode_seconds):
    input_size = os.path.getsize(input_path)
    output_size = report.output_file_bytes
    raw_size = report.raw_float32_bytes
...
def _gather_psnr(original_scene, decoded_scene, kept_count):
    out = {}
    for name in ATTRIBUTE_ORDER:
