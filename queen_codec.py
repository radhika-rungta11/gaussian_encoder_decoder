...
def _train_rvq(data: np.ndarray, stages: int = 2, entries: int = 256, seed: int = 0) -> Tuple[np.ndarray, np.ndarray]:
...
def decode_scene(path: str):
    ...
    if scene.xyz is not None and scene.xyz.shape[0] != num_points:
        raise ValueError(f"Decoded xyz has {scene.xyz.shape[0]} points but header says {num_points}")
